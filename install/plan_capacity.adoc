...
IBM Power systems provide simultaneous multithreading (SMT) levels of 1, 2, 4 or 8 for each core. The required 16 vCPUs can be achieved with different numbers of cores depending on the SMT level.

 *** 2 cores running in SMT-8 -> 16 vCPUs (default for PowerVM systems)
 *** 4 cores running in SMT-4 -> 16 vCPUs (e.g., for non-PowerVM systems)
